...
describe 'Test Folder API' do
  ...
  describe 'Getting folders' do
    it 'HAPPY: should be able to get list of all folders' do
      UCCMe::Folder.create(DATA[:folders][0])
      UCCMe::Folder.create(DATA[:folders][1])
    ...
    it 'HAPPY: should be able to get details of a single folder' do
      existing_folder = DATA[:folders][1]
      UCCMe::Folder.create(existing_folder)
      id = UCCMe::Folder.first.id
      ...
      _(result['data']['foldername']).must_equal existing_folder['foldername']
      _(result['data']['description']).must_equal existing_folder['description']
    end
